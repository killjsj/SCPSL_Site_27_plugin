...
{
    class RuelhintServ : UI.IHintShowHUD
    {
        public static Dictionary<RueDisplay, RuelhintServ> PDH = new Dictionary<RueDisplay, RuelhintServ>();
        public Dictionary<Tag, (Stopwatch, float)> Dur = new Dictionary<Tag, (Stopwatch, float)>();
        public RueDisplay hud;
        static public IHintShowHUD GetPlayerHUD(Player player)
        {
            if (PDH.ContainsKey(RueDisplay.Get(player)))
            {
                return PDH[RueDisplay.Get(player)];
            }
            return new RuelhintServ(player);
        }
        static public bool GetPlayerHUD(Player player, out IHintShowHUD hud)
        {
            if (PDH.ContainsKey(RueDisplay.Get(player)))
            {
                hud = PDH[RueDisplay.Get(player)];
            }
            hud = new RuelhintServ(player);
            return true;
        }
        ...
        public RuelhintServ(Player player)
        {
            this.hud = RueDisplay.Get(player);
        ...
        void IHintShowHUD.AddMessage(string id, string message, float duration, int x, int y)
        {
